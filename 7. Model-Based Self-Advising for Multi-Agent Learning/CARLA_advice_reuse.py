  ...
  # DQN Agent
    ...
    def advising_action(self,state,possible_action):
        ...
        if len(possible_action) == 1:
            ...
            if abs(Q_value[possible_action[0]] - Q_value[possible_action[1]]) > 60:
...
def benchmark1(state):
    if state[0] == 1:
        similar_threshold = 0.13
    else:
        similar_threshold = 0.07
    ...
    for line in lines:
        ...
        sim = calculate_similarity(last_state1,state)

        for z in range(5):
            if sim > sim_5[z]:
                sim_5[z] = sim
                action_5[z] = (action1,current_time1)
                break
    min_time = min([action_5[0][1], action_5[1][1], action_5[2][1], action_5[3][1], action_5[4][1]])
    for x in range(5):
        if min_time == action_5[x][1]:
            return action_5[x][0]
        ...
        while if_run:
            ...
            if args.agent == "Roaming" or args.agent == "Basic":
                ...
                if (agent._vehicle.get_location()).x >50:
                    ...
                    f2 = open("benchmark1_state1.txt", "a")
                    f2.write(content)
                    f2.close()
                ...
                if distance_to_destination<40 and not if_final:
                    ...
                    if current_line != 1:
                        ...
                        content = str(last_state) + '\t' + str(action) + '\t' + str(reward) + '\t' + str(state) + '\t' + str(last_line) + '\t' + str(current_line) + '\t' + str(int(current_time)-int(last_time)) + '\t' + str(done) + '\n'
                        f1 = open("benchmark1_state1.txt", "a")
                        f1.write(content)
                        f1.close()
