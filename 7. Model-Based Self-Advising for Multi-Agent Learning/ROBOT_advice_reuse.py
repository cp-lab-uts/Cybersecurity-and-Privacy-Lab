...
filename = "method(" + currentDT.strftime("%H-%M-%S %Y-%m-%d") + ").txt"
    ...
    def algorithm(self):
        ...
        for i in range(BOT_NUM):
            ...
            if key in observation[i]:
                ...
                observation[i][key] += 1
                if max_pro > 0.4 or observation[i][key] > 10 :
                    ...
                    for j in range(BOT_NUM):
                        if j != i:
                            ...
                            if len(exp_memory[j]) != 0:
                                ...
                                if key in exp_memory[j]:
                                    if len(exp_memory[j][key]) > 5 and len(exp_memory[j][key]) > max_num_obs:
                                        teacher_id = j
                                        max_num_obs = len(exp_memory[j][key])

                    if teacher_id != -1:
                        for slt_action in exp_memory[teacher_id][key]:
                            if slt_action[1] > 0:
                                max_action = max(distribution[teacher_id][key], key=distribution[teacher_id][key].get)
                                
                                break
                        action.append(max_action)

                        # slt_action = random.choice(exp_memory[teacher_id][key])
                        # action.append(slt_action[0])
                    else:
