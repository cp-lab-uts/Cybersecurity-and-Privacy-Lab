...
class DQN():
  ...
  # DQN Agent
    ...
    def create_Q_network(self):
        # network weights
        W1 = self.weight_variable([self.state_dim,n_neural])
        b1 = self.bias_variable([n_neural])

        W2 = self.weight_variable([n_neural,n_neural])
        b2 = self.bias_variable([n_neural])

        W3 = self.weight_variable([n_neural,n_neural])
        b3 = self.bias_variable([n_neural])


        W4 = self.weight_variable([n_neural,self.action_dim])
        b4 = self.bias_variable([self.action_dim])
        # input layer
        self.state_input = tf.placeholder("float",[None,self.state_dim])
        # hidden layers
        h_layer1 = tf.matmul(self.state_input,W1) + b1
        h_layer2 = tf.matmul(h_layer1,W2) + b2
    ...
    def small_train_Q_network(self):
        self.time_step += 1
        # Step 1: obtain random minibatch from replay memory
    ...
    def advising_action(self,state,possible_action):
        ...
        if len(possible_action) == 1:
            ...
            print('Q')
            print(Q_value)
            if Q_value[possible_action[0]] != 0 and Q_value[possible_action[1]] != 0:
                all_q = Q_value[possible_action[0]] + Q_value[possible_action[1]]
                q1 = Q_value[possible_action[0]]/all_q
                q2 = Q_value[possible_action[1]]/all_q
                if np.var([q1,q2]) < 0.03:
                    action = 5
                else:
            ...
            # if abs(Q_value[possible_action[0]] - Q_value[possible_action[1]]) > 100:
            #     action = np.argmax(Q_value)
            # else:
            #     action = 5
        return action
    ...
    def give_advise(self,state):
        ...
        file_name222 = 'full_state.txt'
        f222 = open(file_name222, "r")
        lines = f222.readlines()
        last_cost=0
        ...
        for line in lines:
            ...
            last_state1 = np.array(state1)
            current_state1 = np.array(next_state1)

            reward1 = get_reward(last_line1,current_line1,0,current_time1)

            done1 = int(p4[3])
            if current_time1<70 and self.cal_similarity(last_state1[0:20],state[0:20]) > similar_threshold:
                self.small_perceive(last_state1[0:20],action1,reward1,current_state1[0:20],done1)

            
            

        f111.close()
        self.small_train_Q_network()
        Q_value = self.Q_value.eval(feed_dict = {
          self.state_input:[state]
          })[0]
        return Q_value
...
class small_DQN():
  ...
  # DQN Agent
    ...
    def give_advise(self,state):
        if state[0] == 1:
            similar_threshold = 0.11
        else:
            similar_threshold = 0.06
        file_name222 = 'teacher.txt'
        f222 = open(file_name222, "r")
        lines = f222.readlines()
        last_cost=0
        knowledge_n = 0
    ...
    def self_advise(self,state):
        if state[0] == 1:
            similar_threshold = 0.11
        else:
            similar_threshold = 0.06
        file_name222 = 'student.txt'
        f222 = open(file_name222, "r")
        lines = f222.readlines()
        last_cost=0
        knowledge_n = 0
...
def get_state(all_v):
    ...
    state = [d1_1, d1_2, d1_3, d1_4, d2_1, d2_2, d2_3, d2_4, d3_1, d3_2, d3_3, d3_4, d4_1, d4_2, d4_3, d4_4, d5_1, d5_2, d5_3, d5_4, d6_1, d6_2, d6_3, d6_4, d7_1, d7_2, d7_3, d7_4, d8_1, d8_2, d8_3, d8_4, d9_1, d9_2, d9_3, d9_4]
    return state
...
def possible_action(location):
    dic = {1:[1,2], 2:[1,2], 3:[2], 4:[0,1], 5:[0,1], 6:[1], 7:[1,2], 8:[2], 9:[0], 10:[0], 11:[0], 12:[1], 13:[1], 14:[1]}
    return dic[location]
        ...
        while if_run:
            ...
            if args.agent == "Roaming" or args.agent == "Basic":
                ...
                if (agent._vehicle.get_location()).x >50:
                    ...
                    f.close()
                    dqn.perceive(last_state[0:20],action,reward,state[0:20],done)
                ...
                if distance_to_destination<40 and not if_final:
                    ...
                    p_a = possible_action(current_line)


                    action = dqn.advising_action(state[0:20],p_a)
                    new_content = 'take_action'
                    if action == 5:
                        new_content += '\t ask'
                        
                        small_dqn.chongzhi()
                        #start_core_time = time.clock()
                        Q_value = small_dqn.self_advise(state[0:20])
                        if len(Q_value) == 4:
                            small_dqn.chongzhi()

                            Q_value = small_dqn.give_advise(state[0:20])
                            #end_core_time = time.clock()
                            #print('cpu time')
                            #print(end_core_time-start_core_time)
                            if len(Q_value) == 4:
                                action = dqn.action(state[0:20])
                            else:
                                print(Q_value)
                                print(p_a)
     
                                if Q_value[p_a[0]] > Q_value[p_a[1]]:
                                    action = p_a[0]
                                else:
                                    action = p_a[1]
                        else:
...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        done1 = int(p4[3])
        if current_time1<70:
            dqn.perceive(last_state1[0:20],action1,reward1,current_state1[0:20],done1)
